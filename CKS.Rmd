...
Things to talk about:
  Data is self reported on three levels (each case is a record created by an officer (or group of officers) which includes data reported by a 'suspect'. The dataset as a whole is then turned over by the police)
  Using arrests as a measure for stop success is difficult because of bias (racial bias, bias about who is a criminal, and of course, confirmation bias and 'if I don't arrest this person I was wrong to stop them' bias)
...
Taking a look at days of the week, there do appear to be fewer stops made on certain days of the week, namely Sundays and Mondays. However, there does not appear to be a pattern in the number of stops made over the course of a month. 
...
ggplot(stopFrisk, aes(x = hour, fill = SEARCHED_FLAG)) +
  geom_bar() +
...
ggplot(stopFrisk, aes(x = month, fill = SEARCHED_FLAG)) +
  geom_bar() +
  labs(title = "Stip and Frisk Occurences by Month", 
       fill = "Did the stop lead to an arrest?")

ggplot(stopFrisk, aes(x = day, fill = SEARCHED_FLAG)) +
  geom_bar() +
  labs(title = "Stops By day of the Month")

stopFrisk %>%
  filter(month %in% c(12, 6)) %>%
  mutate(month = as.factor(month)) %>%
  ggplot(aes(x = day, fill = month)) +
    geom_bar() +
    labs(title = "Stops By day of December")
```
